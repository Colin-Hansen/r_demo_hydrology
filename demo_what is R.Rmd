...
```{r compile-raw-data, eval = TRUE, include=FALSE,cache=FALSE}
  ...
  for (i in seq_along(site_names)) {
      ...
      # save the compiled 5-minute rainfall data for each site in a list
      
      # create empty list
      
      if(i==1) { rainfall_data_list <- list(NULL)
      
      } else {
         
        # save to the list
                
        rainfall_data_list[i] <- list(rainfall_data_df)
...
```{r stats-summary-table-1, eval= TRUE, echo = FALSE,include = TRUE, message=FALSE, warning=FALSE, results = 'asis'}
  ...
  for (m in seq_along(csv_name)) {
      ...
      CSV_infile<-file.path(CSV_folder, csv_name[m])
      ...
      stats_folder_2<-'data_out/_2'
      
      # create (if necessary) directory
      if(dir.exists(stats_folder_2) == FALSE) dir.create(stats_folder_2)
     
      
      # delete any previous .CSV files in 'stats_folder_1/2'
      files_to_remove<-dir('data_out/_1',full.names = TRUE)
      file.remove(files_to_remove)
      
      files_to_remove<-dir('data_out/_2',full.names = TRUE)
      file.remove(files_to_remove)
